        ...
        if re.match(r'^\d\.\d\.\d\.\d\.\d$', text):
            ...
            try:
                ...
                with open(error_log_path, "w") as error_log_file:
                    process = subprocess.Popen(["python3", "self.py", str(m.chat.id), str(API_ID), API_HASH, Helper_ID], cwd=f"selfs/self-{m.chat.id}", stdout=error_log_file, stderr=subprocess.STDOUT)
            ...
            if get_data(f"SELECT * FROM user WHERE id = '{user_id}' LIMIT 1") is not None:
                if os.path.isfile(f"sessions/{user_id}.session-journal"):
                    ...
                    if user_data["self"] != "active":
                        ...
                        with open(error_log_path, "w") as error_log_file:
                            process = subprocess.Popen(["python3", "self.py", str(user_id), str(API_ID), API_HASH, Helper_ID], cwd=f"selfs/self-{user_id}", stdout=error_log_file, stderr=subprocess.STDOUT)
                        await asyncio.sleep(10)
                        if process.poll() is None:
                            if os.path.isfile(error_log_path):
                                os.remove(error_log_path)
